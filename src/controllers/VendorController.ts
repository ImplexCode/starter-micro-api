...
export const GetVendorProfile = async (req: Request,res: Response, next: NextFunction) => {
    ...
    if(user){
       ...
       const existingVendor = await Vendor.findById(user._id).populate('foodType');
...
export const UpdateVendorProfile = async (req: Request,res: Response, next: NextFunction) => {
    ...
    const { foodType, name, address, phone, lat, lng} = req.body;
...
export const AddFood = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if(user){
       ...
       if(vendor !== null){
            ...
            if(!files){
                return res.json({'message': 'Please upload at least one image'});
            }
            if(!name || !description || !category || !foodType || !readyTime || !price){
                return res.json({'message': 'Please provide all the information'});
            }
...
export const GetFoods = async (req: Request, res: Response, next: NextFunction) => {
    ...
    //    const foods = await Food.find({ vendorId: user._id}).populate('category');
       const foods = await Vendor.findById(user._id).populate('foods').populate('foodType');
...
export const DeleteFood = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if(!foodId) return res.json({'message': 'Food id is required!'});
    
    if(user){

       try{
        // const food = await Food.deleteOne({ _id: foodId});
        const food = await Vendor.updateOne({ _id: user._id}, { $pull: { foods: foodId }});
        
        return res.json(food);
       }
       catch(err){
           console.log(err);
           return res.json({'message': 'Unable to delete food!'});
       }
    }
    return res.json({'message': 'Foods not found!'})
}
...
export const VendorGetCategory = async (req: Request, res: Response, next: NextFunction) => {
...
export const VendorGetCategoryByID = async (req: Request, res: Response, next: NextFunction) => {
